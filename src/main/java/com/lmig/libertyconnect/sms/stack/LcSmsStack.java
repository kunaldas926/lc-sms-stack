...
public class LcSmsStack extends Stack {
	...
	public LcSmsStack(final Construct parent, final String id, final StackProps props, final Args ARGS) {
		...
		final Function smsConnectorLambda = Function.Builder.create(this, ARGS.getPrefixedName("lc-sms-connector-lambda"))
				...
				.environment(envsMap)
				.vpc(Vpc.fromLookup(smsProcessorLambda, ARGS.getPrefixedName("lc-sms-db-processor-vpc"), VpcLookupOptions.builder().build()))
		 ...
		 final Function smsDbConnectorLmbda = Function.Builder.create(this, ARGS.getPrefixedName("lc-sms-db-connector-lambda"))
				...
				.environment(envsMap)
				.vpc(Vpc.fromLookup(smsProcessorLambda, ARGS.getPrefixedName("lc-sms-db-connector-vpc"), VpcLookupOptions.builder().build()))
				/*.fromVpcAttributes(this, ARGS.getPrefixedName("lc-sms-connector-vpc"), VpcAttributes.builder()
				.vpcId("vpc-6d3d8b0a")
				.availabilityZones(Arrays.asList("ap-southeast-1a", "ap-southeast-1b"))
				.privateSubnetIds(Arrays.asList("subnet-ea5a228d", "subnet-bd056df4"))
